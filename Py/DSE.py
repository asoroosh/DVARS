...
#d = kwargs.get('d', None)
def DSE(Y,I,T,I1,imobj,pth,fn,rmvIdx,Dvar,NewDir2DSE):
...
def DVARS_Calc(Y,I,T,I1,imobj,pth,fn,Dvar,NewDir2DSE):    
    print('Not Ready Yet!')
    #defaults:
    WhichExpVal = 'median';    
    WhichVar    = 'hIQRd';
    ...
    def X2p0(x,m,s):    return((X2stat(x,m,s)-X2df(m,s))/sqrt(2*X2df(m,s)));
    ...
    DVMean = {'sig2bar':np.mean(Rob_S_D**2,axis=0),'sig2median':np.median(Rob_S_D**2,axis=0),
               'median':np.median(DVARS2,axis=0),'sigbar2':np.mean(Rob_S_D,axis=0)**2,
               'xbar':np.mean(DVARS2,axis=0)};        
    DVVar  = {'S2':np.var(DVARS2,axis=0), 'IQRd':(1/dd*M_Z**(1/dd-1)*IQRsd(Z))**2,
