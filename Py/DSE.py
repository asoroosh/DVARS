...
# These are all sorted! Everything is fine and sound! I just need to do a bit more on item 1
# 1) Fix the stat and prac significance. 
# 2) Check the actuall values for DVARS. They should be identical right? Unless there is an indexing problem somewhere. 
# 3) Check the p-values and z-scores, they should be roughly similar due to IQR difference 
# 4) Sort out a DVARS written output somehow
# 
#
...
def DSE_Calc(Y,\
    ...
    if imobj != '' and DSEImage:
        ...
        vimg=''
        print('The DSE image option was left False, if you need them trigger option DSEImage=.')
...
def DVARS_Calc(Y,\
    ...
    print(str(np.size(Hval_stat)) + ' of volumes are statistically significant.')

    Hval_practical = np.where(DeltapDvar>DeltapDvarThr)
    print(str(np.size(Hval_practical)) + ' of volumes are statistically significant.')
