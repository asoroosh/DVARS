function [DVARS,Stat]=DVARSCalc_octave(V0,varargin)
...
if verbose
%     ***array2table is not available in Octave***
...
%     disp(array2table(Va,'VariableNames',VarNms));...
    ...
    disp(['Means: '])
    cnt = 1;
    for MeanNmsLab = MeanNms
        disp([MeanNmsLab{1} '   :      ' num2str(Mn(cnt))]); 
        cnt = cnt+1;
    end

    disp(['Variances: '])
    cnt = 1;
    for VarNmsLab = VarNms
        disp([VarNmsLab{1} '    :       ' num2str(Va(cnt))]); 
        cnt = cnt+1;
    end
